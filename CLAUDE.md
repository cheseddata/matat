...
- **Context for the recovery flow**: Mrs. Rosen reports conversion rates dropped well before anyone realised the issue. A salesperson called one donor and discovered their Techloq filter had silently blocked the matatmordechai.org link. The recovery page exists to clear that backlog quickly.

### 2026-06-26 (snapshot from embedded /donate too + bounce sweep)
- **PI webhook now writes a snapshot** (`_write_contact_snapshot_from_pi`) using metadata from our embedded `/donate` form plus billing_details fallback. Idempotent: when both PI and `checkout.session.completed` fire (Stripe Payment Link path), the session handler enriches the row the PI handler already created. So every donation gets exactly one snapshot regardless of which entry point spawned it.
- **`flask sweep-bounces`** new CLI command (`app/services/bounce_sweep.py`). Walks the synced Exchange inbox for the last N hours (default 72), recognises DSN/NDR messages by subject regex (`undeliverable`, `delivery (status|notification|failure)`, etc.) plus sender regex (`MAILER-DAEMON`, `postmaster`, …), pulls the failed recipient out of `Final-Recipient:` / `Original-Recipient:` headers or via a first-non-self email fallback. For each match: flips `snapshot.receipt_bounced=true`, records the bounce subject, and if `donor.email` differs from the bounced address, re-renders the receipt PDF and sends to the donor's main email. Sets `receipt_fallback_used=true` on success.
- **First live run** (`--hours 168 --dry-run`) picked up 2 actual bounce messages in the last week; both `no_match` because no snapshots exist for those donations yet (snapshot table was just introduced). Wire into cron for ongoing recovery (suggested: `5,35 * * * * cd /var/www/matat && /var/www/matat/venv/bin/flask sweep-bounces --hours 6`).
...
- The static template wiring also passes `salesperson` and `stripe_fallback_url` into the render context, so the `{% if salesperson and salesperson.ref_code %}?client_reference_id={{ salesperson.ref_code }}{% endif %}` suffix actually fires for system-generated link emails (previously only worked in the salesperson custom-message flow).

### 2026-06-26 (filter-fallback Stripe Payment Link + webhook race fix + commission attribution)
- **New donation-link email fallback block.** Every salesperson + system donation-link email now carries a secondary "Donate via Stripe" button under the matat-hosted link. The Stripe Payment Link (`https://donate.stripe.com/dRm5kE2IRb2AdC16I22cg02`) lives on `*.stripe.com`, which Techloq/Netspark/Rimon/etc. all whitelist by default, so donors whose filters haven't yet whitelisted `matatmordechai.org` can still complete the donation. Amber-bordered box with a professional explanation ("It has come to our attention that certain internet content filters may not yet have updated their approved-sites list..."). Patched into both `donation_link_en.html`/`donation_link_he.html` static templates and the inline HTML in `send_custom_donation_link_email`.
- **Stripe-side setup**: Programmatically created Product `prod_UmBU0y1DzIQspd`, custom-amount Price `price_1TmdEhLpYYqBobrrM86hI37c` (USD, $1 minimum, no preset so the input box starts empty), PaymentLink `plink_1TmdEhLpYYqBobrrKF3BqRE7` with `submit_type='donate'` → Stripe auto-routes to `donate.stripe.com` (not `buy.stripe.com`) which reads as more on-brand for nonprofit. Custom thank-you message on the post-payment confirmation page.
- **Commission attribution for fallback donations**: appended `?client_reference_id={ref_code}` to the fallback URL whenever the email is being sent by a salesperson. New `handle_checkout_session_completed` webhook reads `client_reference_id` and credits the matching User by `ref_code`, stamping `attributed_via=client_reference_id` + `source=filter_fallback_link` on `donation.stripe_metadata` so a later audit can see which donations came through the fallback path. Webhook endpoint `we_1TEvlVLpYYqBobrrYzsrnqMo` updated via API to enable `checkout.session.completed`.
- **Race-condition fix in `handle_charge_succeeded`**: discovered during testing (donation #6737 silently skipped its receipt). Stripe fires `payment_intent.succeeded` and `charge.succeeded` ~200ms apart and gunicorn routes them to different workers — the charge handler can run before the PI handler has committed the donation row, so the "no donation found" branch bails without creating a receipt. Now retries the donation lookup 5 × 300ms before giving up. Same retry pattern added to the new `checkout.session.completed` handler. Manually generated receipt **MM-2026-00401** for #6737 retroactively.
...
- Donor lookup at the top reuses `/admin/api/donors/search` — same live-search pattern as the manual-donation form, so an existing donor's name/email/phone/address pre-fill in one click.
- **Sticky black nav bar across every Matat page** (`base.html`): `.nav { position: sticky; top: 0; z-index: 1000 }`. Closes a long-standing usability paper-cut on long pages — the operator can always reach the menu without scrolling back up. Affects every page that extends `base.html`, including the Weddings list which the operator specifically called out.
- **New always-visible processor tab strip** sticky just below the nav (`top: 50px`). Renders for any authenticated user with at least one allowed processor. Tabs: All / each enabled processor the user is allowed to view (respects `User.allowed_processors`) / a green "💳 Charge Card" shortcut on the far end for admins. Each tab links to `/admin/donations?processor=<code>`, so it's a quick filter from anywhere — not just the donations list. Active state highlights when on `/admin/donations` with a matching `?processor=` arg.
- ZTorm and Gemach pages are intentionally excluded — they use their own `ztorm_base.html` / `gemach_base.html` shells. They each already have a "back to Matat" toggle, so no change needed there.
- **Context processor `nav_processors`** added to `app/utils/i18n.py`'s `inject_i18n` — surfaces the user's allowed-processors list to every template without requiring each route to compute it. Dies quietly for unauthenticated requests (empty list).
- **Two admin routes restored to nav** that the multi-office refactor had stubbed to `href="#"`: `admin.new_check_donation` (Manual Donation) in the salesperson nav, and `admin.donation_permissions` in the admin Settings dropdown. Both routes were already restored earlier in the day; just the nav links still pointed at stubs. Also added a "💳 Charge Card" entry to the admin Donations dropdown.
