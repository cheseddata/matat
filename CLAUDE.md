...
### 2026-04-29 (multi-office segregation + website→Access first test)
- **`donors.owner_user_id` column added** (Alembic `a8d501f9c5b3` → down_revision `0fc24eb63f94`). FK to `users.id` ON DELETE SET NULL, indexed. Each donor is "owned" by one user account; admin/salesperson views will filter by this field so each office sees only their own contacts.
- **CLI: `flask backfill-donor-owner --user-id <id>`** (`app/cli.py`). Sets `owner_user_id` on every donor whose value is NULL. Skips soft-deleted donors by default. Used at rollout to assign all 2,488 active prod donors to user_id=4 (Gittle Goldblum). Future donors get owner_user_id assigned at creation time.
- **Website→Access write path proven end-to-end** for Mendy Hirsch's two Stripe donations (MM-2026-00122 / MM-2026-00123, both 2026-03-26). Inserted: ZTorm Kabalot 3400+3401 (mispar 9003999/9004000), Tashlumim 36819+36820 under Truma 3370, Truma aggregates updated (shulam_d 7000→10000, shulam_s 26080.5→35410.5, last_shulam=03/26/2026), Gmach Tnuot rows 197835+197836 under card_no 4087. New scripts in `sync/`: `add_website_donation.ps1` (ZTorm Kabala+Tashlum), `add_tashlum_only.ps1` (recovery for partial run), `add_gmach_tnua.ps1` (Gmach side, ADO Recordset for Hebrew/Unicode safety), `attach_receipt_pdf.ps1` (PDF reference). PDFs scp'd from prod to `C:\matat\website_receipts\`; index in `instance/website_receipts_index.json` maps num_kabala→PDF path. New route `/gemach/receipts/website/<num_kabala>` serves PDFs inline; member-history page renders 📄 link next to matching kabala rows. **Caveat:** ZTorm `user` workgroup account has INSERT but not UPDATE on Kabalot, so we couldn't write the PDF path back into Access — the index stays sandbox-side. Truma aggregates were updated successfully (different table/permission).
- **Memory kill-switch on gmach launcher** (`access_mirror/gmach/gmach.vbs`): added `taskkill /F /IM chrome.exe` and `msedgewebview2.exe` before opening Access. Operator's 8 GB PC pages to disk when Access runs alongside Chrome (~1 GB) + WebView2 leftovers (~400 MB). AnyDesk runs as `AnyDesk.exe` so the remote-control session is unaffected. Marked TEMPORARY in the file — remove when RAM is upgraded.
